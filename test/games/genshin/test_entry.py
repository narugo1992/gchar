...
from gchar.games.genshin.__main__ import cli


@pytest.mark.unittest
class TestGamesGenshinEntry:
    def test_version(self):
        result = simulate_entry(cli, ['gchar.games.genshin', '-v'])
        assert result.exitcode == 0
        assert __VERSION__ in result.stdout
        assert 'genshin' in result.stdout

    @skipUnless(not OS.windows, 'Non-windows required.')
    def test_update_full_5(self, no_index_json):
        result = simulate_entry(cli, ['gchar.games.genshin', 'update', '-n', '5'])
    ...
    def test_update_full(self, no_index_json):
        result = simulate_entry(cli, ['gchar.games.genshin', 'update'])
        assert result.exitcode == 0, f'Exitcode is {result.exitcode}.{os.linesep}' \
                                     f'This is stdout:{os.linesep}{result.stdout}{os.linesep}' \
                                     f'This is stderr:{os.linesep}{result.stderr}{os.linesep}'
        assert os.path.exists(no_index_json)
